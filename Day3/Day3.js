    ...
    //  console.log(firstInt[i] + " Direction " + direction + " Movement " + movement);
      ...
      switch(direction)
      {
        ...
        for(let j = 0; j < movement; j++)
        {
            ...
            firstPath.push(x +"," + parseInt(y+movement-j));
            cumulativeStepCounter.push(cumulativeStepCounter[cumulativeStepCounter.length-1] + 1);
        ...
        for(let j = movement; j > 0; j--)
        {


           firstPath.push(parseInt(x+j-movement) +"," + y);
           cumulativeStepCounter.push(cumulativeStepCounter[cumulativeStepCounter.length-1] + 1);
...
//        console.log(x);
        for(let j = movement; j > 0 ; j--)
        {
           ...
           firstPath.push(parseInt(x+movement-j) +"," + y);
           cumulativeStepCounter.push(cumulativeStepCounter[cumulativeStepCounter.length-1] + 1);
  ...
  //cumulativeStepCounter.push(Math.abs(finalmovement));

for (let k = 0; k < firstPath.length -1 ; k++ )
{
  if (firstPath[k].includes("MAIN") == true )
  {
//    console.log(firstPath[k]);
    firstPath.splice(k,1);
  }
}
firstPath[firstPath.length -1] = firstPath[firstPath.length -1].slice(0,firstPath[firstPath.length -1].length -4);
firstPath.splice(0,1);
//console.table(firstPath);

}
console.log("both tables sorted");
//console.table(firstPath);
//console.table(secondPath);
let sharedLocations = [];

for(let l =0; l < firstPath.length; l++)
{

  for(let a = 0; a < secondPath.length; a++)
  {
    if (firstPath[l] == secondPath[a] && firstPath[l] != "0,0")
    {
        sharedLocations.push(firstPath[l]);
    }
  }
}

//console.table(sharedLocations);
let finallengths = [];
for(let f =0; f < sharedLocations.length; f++)
{
  let manhattan = sharedLocations[f].split(",");
  let finallength = Math.abs(parseInt(manhattan[0])) + Math.abs(parseInt(manhattan[1]));
  finallengths.push(finallength);
}

//finallengths.sort(function(a, b){return a-b});
console.table(finallengths);


console.log(finallengths[0]);
console.table(firstPath);
console.table(cumulativeStepCounter1);
console.table(secondPath);
console.table(cumulativeStepCounter2);

console.table(sharedLocations);

let shortestCombinedPath = 999999;
//part 2 , inplement cumulativestepcounter to paths
//find count for path A and B for each shared value, add them , add to array , sort by smallest
for(let fl = 0; fl < sharedLocations.length; fl++)
{
  let firstindex = parseInt(cumulativeStepCounter1[firstPath.indexOf(sharedLocations[fl])]);
  console.log("Path 1 " + firstPath.indexOf(sharedLocations[fl]));
//  console.log(firstPath[206]);

  console.log(firstindex);

  let secondindex = parseInt(cumulativeStepCounter2[secondPath.indexOf(sharedLocations[fl])]);
console.log("Path 2 " + secondPath.indexOf(sharedLocations[fl]));
  console.log(secondindex);
  let path = parseInt(firstindex + secondindex);
  console.log(path);
  if(path < shortestCombinedPath)
  {
    shortestCombinedPath = path;
  }
}

console.log(shortestCombinedPath);
