...
//let secondString = "U62,R66,U55,R34,D71,R55,D58,R83";

//let firstString = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
...
test(firstPath, firstInt);

test(secondPath, secondInt);

function test(firstPath, firstInt) {
...
a
  ...
  firstPath[firstPath.length - 1] = firstPath[firstPath.length - 1].slice(
    0,
    firstPath[firstPath.length - 1].length - 4
  );
...
for (let l = 0; l < firstPath.length; l++) {
  ...
  for (let a = b; a < secondPath.length; a++) {
...
for (let f = 0; f < sharedLocations.length; f++) {
  ...
  let finallength =
    Math.abs(parseInt(manhattan[0])) + Math.abs(parseInt(manhattan[1]));
...
for (let fl = 0; fl < sharedLocations.length; fl++) {
  let firstindex = firstPath.indexOf(sharedLocations[fl]);
  ...
  let secondindex = secondPath.indexOf(sharedLocations[fl]);
