...
public class Program {
	...
	public static void main(String[] args) {
		/*
		 * Part 2 Nested Look to search every variation of noun and verb to find desired
		 * output
		 */
...
//	 int[] intcode = new int[] {3,3,1105,-1,9,1101,0,0,12,4,12,99,1};
		...
		int[] intcode = new int[] { 3, 21, 1008, 21, 8, 20, 1005, 20, 22, 107, 8, 21, 20, 1006, 20, 31, 1106, 0, 36, 98,
...
// new opcode 4 takes value of parameter and outputs
		...
		// Loop through intcodes , every 4th integer is an opcode , 0th int is opcode ,
		// followed by 2 parameters and output location
		...
		for (int instructionPointer = 0; instructionPointer <= intcode.length; instructionPointer += jump) {
			...
			// Array values are treated as memory addresses, so the value isn't itself used
			// in the calculation rather the value at the address of the initial value
...
//    	  System.out.println(intcode[20]);
			...
			case 1:
				...
				if (Cflag == 1 && Bflag == 1) {
					...
					intcode[intcode[instructionPointer + 3]] = bb + cc;
					// System.out.println("REACHED");
				} else if (Cflag == 1) {

					int cc = intcode[C];
					// System.out.print("cc is :" + cc);
					intcode[intcode[instructionPointer + 3]] = intcode[intcode[instructionPointer + 2]] + cc;
				}
					...
					int bb = intcode[B];
					// System.out.print("BB is :" + bb);
					intcode[intcode[instructionPointer + 3]] = intcode[intcode[instructionPointer + 1]] + bb;
				} else {

					intcode[intcode[instructionPointer + 3]] = intcode[intcode[instructionPointer + 2]] +intcode[intcode[instructionPointer + 1]];
				}

				// System.out.println(" A FLAG " + Aflag + " BFLAG " + Bflag + " CFLAG " +
				// Cflag);
			...
			case 5: // jump if true
				...
				System.out.println("C5 " + value + " " + C + " " + B + " " + A);
				System.out.println("C5 Flags " + "C :" +  Cflag + " B: " + Bflag + " A: " + Aflag);
			...
			case 6: // jump if false
				...
				System.out.println("INTIIAL " + value + " " + C + " " + B + " " + A);
				System.out.println("C6 Flags " + "C :" +  Cflag + " B: " + Bflag + " A: " + Aflag);
				...
				} else
				{
					...
					if (intcode[intcode[instructionPointer + 1]] == 0) {
					instructionPointer = intcode[intcode[instructionPointer + 2]];
					System.out.println("instpoint4 " + instructionPointer);
					jump = 0;
				} else {
					System.out.println("No Jump");
					jump = 3;
			...
			case 7: // less than
				...
				if (Cflag == 1 && Bflag == 1) {
					...
					if (cc < intcode[intcode[instructionPointer + 2]]) {
						intcode[intcode[instructionPointer + 3]] = 1;
					} else {
						intcode[intcode[instructionPointer + 3]] = 0;
					}
					...
					int bb = intcode[B];
					if (intcode[intcode[instructionPointer + 1]] < bb) {
						intcode[intcode[instructionPointer + 3]] = 1;
					} else {
						intcode[intcode[instructionPointer + 3]] = 0;
					}
			...
			case 8:
				...
				System.out.println("C8 Flags " + "C :" +  Cflag + " B: " + Bflag + " A: " + Aflag);
				...
				try {
					if (Cflag == 1 && Bflag == 1) {
						...
						if (bb == intcode[intcode[instructionPointer + 1]]) {
							System.out.println("hi");
							intcode[intcode[instructionPointer + 3]] = 1;
						} else {
				...
				System.out.println("Final output: " + intcode[intcode[A]] + " " + intcode[intcode[instructionPointer + 3]]);
